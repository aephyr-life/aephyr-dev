...
object MagicLinkCommandEndpoints:
  ...
  val requestMagicLink: Endpoint[Unit, MagicLinkCreationRequest, ErrorDto, MagicLinkCreationResponse, Any] =
    endpoint.post
      .in("api" / "auth" / "link")
      .in(jsonBody[MagicLinkCreationRequest].example(MagicLinkCreationRequest(User.EmailAddress("user@example.com"))))
      .out(jsonBody[MagicLinkCreationResponse].example(MagicLinkCreationResponse(
