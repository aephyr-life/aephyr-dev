...
object K_Aggregates {
    ...
    fun hero(
        ...
        val totalEnergyKJ = K_KiloJoule(totalEnergyKJValue)

        // Macros total (null-safe)
        val totalMacros = entries.fold(K_Macros.zero()) { acc, e ->
            val m = e.macros ?: return@fold acc
            K_Macros(
                protein = K_Decigram(acc.protein.value + m.protein.value),
                fat = K_Decigram(acc.fat.value + m.fat.value),
                carb = K_Decigram(acc.carb.value + m.carb.value)
            )
        }
        ...
        return K_AggregateHero(
            ...
            totalEnergyKJRounded = K_KiloJoule(roundedKJ),
    ...
    private fun macrosToKJ(m: K_Macros): Int {
